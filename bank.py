...
# A better method uses floor division (//) and modulo (%).

    # // floor division gives the euro amount rounded down to the nearest whole number.
    euro = (amount1 + amount2) // 100

    # % modulo will give the remainder of the sum of amount 1 and amount 2 divided by 100. This is the cent amount.
    cent = (amount1 + amount2) % 100

    # Print the required output in €euro.cent
    print(f'The sum of these is €{euro}.{cent}')

# The error message will print if an whole number is not entered.
except ValueError:
